...
-- =====================================================
-- 0. 清理可能已存在的字段和表（避免重复添加错误）
-- =====================================================

-- 删除可能已存在的字段（删除字段会自动删除相关索引）
SET @exist_is_pinned = (SELECT COUNT(*) FROM information_schema.COLUMNS 
    WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'group_members' AND COLUMN_NAME = 'is_pinned');
SET @sql_drop_is_pinned = IF(@exist_is_pinned > 0, 'ALTER TABLE `group_members` DROP COLUMN `is_pinned`;', 'SELECT ''is_pinned 不存在，跳过删除'';');
PREPARE stmt FROM @sql_drop_is_pinned;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;

SET @exist_is_disturb_free = (SELECT COUNT(*) FROM information_schema.COLUMNS 
    WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'group_members' AND COLUMN_NAME = 'is_disturb_free');
SET @sql_drop_is_disturb_free = IF(@exist_is_disturb_free > 0, 'ALTER TABLE `group_members` DROP COLUMN `is_disturb_free`;', 'SELECT ''is_disturb_free 不存在，跳过删除'';');
PREPARE stmt FROM @sql_drop_is_disturb_free;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;

SET @exist_chat_background = (SELECT COUNT(*) FROM information_schema.COLUMNS 
    WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'group_members' AND COLUMN_NAME = 'chat_background');
SET @sql_drop_chat_background = IF(@exist_chat_background > 0, 'ALTER TABLE `group_members` DROP COLUMN `chat_background`;', 'SELECT ''chat_background 不存在，跳过删除'';');
PREPARE stmt FROM @sql_drop_chat_background;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;

SET @exist_clear_history_time = (SELECT COUNT(*) FROM information_schema.COLUMNS 
    WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'group_members' AND COLUMN_NAME = 'clear_history_time');
SET @sql_drop_clear_history_time = IF(@exist_clear_history_time > 0, 'ALTER TABLE `group_members` DROP COLUMN `clear_history_time`;', 'SELECT ''clear_history_time 不存在，跳过删除'';');
PREPARE stmt FROM @sql_drop_clear_history_time;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
...
SET @exist_index = (SELECT COUNT(*) FROM information_schema.STATISTICS 
    WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'group_members' AND INDEX_NAME = 'idx_is_pinned');
SET @sql_create_index = IF(@exist_index = 0, 
    'CREATE INDEX `idx_is_pinned` ON `group_members`(`user_id`, `is_pinned`) USING BTREE COMMENT ''用户置顶群聊索引'';', 
    'SELECT ''索引 idx_is_pinned 已存在，跳过创建'';');
