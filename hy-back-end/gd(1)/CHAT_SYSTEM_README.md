...
1. **用户A添加用户B为好友**
```bash
curl -X POST http://localhost:8082/api/user-chat/friends/add \
...
2. **用户B处理好友申请**
```bash
curl -X POST http://localhost:8082/api/user-chat/friends/handle \
  -H "Content-Type: application/json" \
...
4. **用户B查看聊天记录**
```bash
curl "http://localhost:8082/api/user-chat/messages/history?userId=2&friendId=1&page=1&size=20"
```

5. **用户A创建群聊并邀请好友**
```bash
curl -X POST http://localhost:8082/api/user-chat/group/create-with-friends \
  -H "Content-Type: application/json" \
