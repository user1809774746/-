...
const ws = new WebSocket(`ws://localhost:8080/ws/chat/native?userId=${userId}`);

// 或通过Header传递（需要后端支持）
const ws = new WebSocket('ws://localhost:8080/ws/chat/native');
...
```javascript
const userId = 123;
const socket = new SockJS(`http://localhost:8080/ws/chat?userId=${userId}`);
```
...
export default {
    setup() {
        ...
        function connect() {
            ...
            ws.value = new WebSocket(`ws://localhost:8080/ws/chat/native?userId=${userId}`);
...
function useChatWebSocket(userId) {
    ...
    useEffect(() => {
        // 连接WebSocket
        ws.current = new WebSocket(`ws://localhost:8080/ws/chat/native?userId=${userId}`);
