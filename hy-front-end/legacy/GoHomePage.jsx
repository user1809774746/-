...
import { API_CONFIG, getFriendsList, shareTravelPlanToAI, getCurrentUserId, sendMessage } from "../api/config";

import SwipeableItem from "./SwipeableItem";
import AiFloatingButton from "./AiFloatingButton";

export default function MyTravalPlanPage({onBack,onNavigateToPlanPostDetail,trip,onNavigateToAddActivity,extraAttractions,onPlanRoute,userLocation,onNavigateToAi,onNavigateToChat}) {
  ...
  'bg-lightLavender',
    ...
    const fetchTravelPlanAttractions = async (planId) => {
      ...
      try {
        ...
        if (response && response.code === 200 && response.data && Array.isArray(response.data.attractions)) {
          console.log('解析到的景点数量:', response.data.attractions.length);
          if (response.data.attractions.length > 0) {
            console.log('第一条景点示例:', response.data.attractions[0]);
          }
          setPlanAttractions(response.data.attractions);
        } else {
    ...
    const handleShareToAI = async () => {
      ...
      try {
        // 显示选择分享目的的模态框
        const purpose = await showSharePurposeModal();
        if (!purpose) {
          return; // 用户取消
        }
        ...
        if (response && response.code === 200) {
          ...
          // 将AI回复存储到localStorage供AI页面显示
          if (response.data.aiReply) {
            localStorage.setItem('aiDialogInput', response.data.aiReply);
          }
    ...
    useEffect(() => {
      ...
      const loadMapScript = () => {
        return new Promise((resolve, reject) => {
          ...
          script.src = amapConfig.getApiUrl([]);
          script.onload = () => {
            if (window.AMap) {
              resolve(window.AMap);
            } else {
   ...
   return (
    <>
        ...
        <div className="flex flex-col min-h-screen bg-white">
            ...
            <div className="w-full h-16 bg-macaron-blue-100 flex flex-row items-center">
                <button onClick={onBack} className="mr-3">
                  <i className="text-xl text-gray-600 fa-solid fa-arrow-left"></i>
                </button>
                <div className="font-bold text-xl absolute left-7 top-4">
                  {displayData.travelPlan?.destination || '加载中...'}
                  {trip?.name}
                </div>
                <img
                  src="./imge/分享.png"
            ...
            <div className="mt-6 ml-5 font-bold text-gray-400" onClick={()=>setShowCalendar(true)}>
            ...
            <div className="pt-4 pb-4">
                                        ...
                                        <div className="mt-3">
                                            ...
                                            <div className="space-y-0">
                                                {day.activities.map((activity, actIndex) => (
                                                 ...
                                                 <div
                                                   className="font-bold mb-8 mt-3 ml-6 text-lg cursor-pointer"
                ...
                <div className="mx-4 mt-4 space-y-3 mb-16">
                  <div
                    className="bg-yellow rounded-lg shadow-sm overflow-hidden h-20 cursor-pointer"
                    onClick={() => setShowExtraDayExpanded(!showExtraDayExpanded)}
                  >
                    <div className="font-bold ml-3 mt-5">再增加一天旅行计划吧</div>
                  </div>
                  {showExtraDayExpanded && (
                    <div
                      className="font-bold mb-8 mt-3 ml-6 text-sm cursor-pointer"
                ...
                {/* {extraAttractions && extraAttractions.length > 0 && (
                  ...
                  {/* 上传图片 */}
              <div className="w-full h-20 mt-10 ml-5">
                <h3 className="text-lg font-bold mb-2">我的图库</h3>
                <div className="flex flex-row">
                  <div className="border border-solid rounded-lg w-20 h-20 cursor-pointer" onClick={handleAddImageClick}>
                    <img src="./imge/加号.png" className="w-10 h-10 mt-5 ml-5"/>
                  </div>
                  {lastUploadedImageUrl && (
                    <div className="border rounded-lg w-20 h-20 overflow-hidden cursor-pointer" onClick={handleOpenImageGallery}>
                      <img src={lastUploadedImageUrl} className="w-full h-full object-cover"/>
                    </div>
                  )}
                </div>
                <input
                ...
                {/* 酒店推荐 */}
                <div className="mx-4 mt-10 space-y-3 mb-16 ">
            ...
            {showImageGallery && (
              <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                <div className="bg-white rounded-lg p-4 max-w-md w-full max-h-[80vh] overflow-y-auto mx-4">
                ...
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-end justify-center z-50">
                  ...
                  <div className="bg-white rounded-t-2xl p-6 w-full max-w-md">
                    <div className="flex justify-between items-center mb-4">
                        ...
                        <input
                          type="text"
                          className="w-full font-bold px-3 py-2 text-lg bg-transparent border-none focus:outline-none focus:ring-0"
                          value={editLocation}
                    ...
                    <div className="w-full flex flex-col items-stretch justify-center space-y-4">
                      <div className="flex flex-row space-x-3">
                        <div className="flex-1">
                         <div className="flex flex-row">
                          <div className="text-sm py-2 w-full">时间：</div>
                          <input
                            type="text"
                            readOnly
                            onClick={handleOpenTimePicker}
                            className="w-full mr-5 py-2 border border-gray-300 text-sm bg-transparent border-none focus:outline-none focus:ring-0"
                            value={editTime}
                        ...
                        <div className="flex-1">
                          <div className="flex flex-row">
                          <div className="text-sm w-full py-2">费用:</div>
                          <input
                            type="number"
                            min="0"
                      ...
                      <div
                        className="flex flex-row w-full h-[60px] items-center mt-4 cursor-pointer"
                      ...
                      >
                        ...
                        ></div>
                        <div className="ml-3 flex-1 bg-gray-100 h-full flex items-center px-3 text-sm text-gray-700">
                      ...
                      <div className="flex flex-row">
                        <div
                          className="text-sm w-20 py-2 px-3 border-2 border-solid rounded-lg text-center cursor-pointer"
                          onClick={handleNavigateToMapFromActivity}
                        >
                          导航
                        </div>
                        <img
                          className="mt-2 ml-10 w-6 h-6 cursor-pointer"
                          src="./imge/垃圾桶.png"
                    ...
                    <div className="w-full h-auto flex flex-col border rounded-lg p-3">
                      <div className="w-full bg-white flex flex-row items-center space-x-3">
                        <div className="bg-macaron-blue-300 w-10 h-10 border rounded-lg flex items-center justify-center">
                          <img src="./imge/搜索(1).png" className="w-6 h-6 mr-2"/>
                        </div>
                        <input
